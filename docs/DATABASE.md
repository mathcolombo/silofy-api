...
|:-------------|:--------------|:------------------|:---------------------------------|
| Id           | INT           | PK                | Identificador único (EntityBase) |
...
| UserId  | INT          | FK -> Users(Id) | Dono da conta                         |
...
| AccountId     | INT           | FK -> Accounts(Id) | Conta física de origem           |
...
| UserId	INT | FK -> Users(Id) | NULL         | para padrões, Preenchido para custom |
...
|:----------|:-----|:-----------------------|:-------------------------------|
| Id        | INT  | PK	Identificador único |
| SiloId    | INT  | FK -> Silos(Id)        | O silo que contém a categoria  |
| CategoryId| INT  | FK -> Categories(Id)   | A categoria vinculada          |
| AccountId | INT  | FK -> Accounts(Id)     | Usada para a Unique Constraint |
...
| AccountId  | INT           | FK -> Accounts(Id)   | Conta afetada                          |
| CategoryId | INT           | FK -> Categories(Id) | Classificação do gasto                 |
| UserId     | INT           | FK -> Users(Id)      | Filtro de segurança por usuário        |
